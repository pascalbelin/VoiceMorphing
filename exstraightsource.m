...
% Manual F0 tracking fix procedure - 2nd March 2009
...
% f0v(f0v > 8000) = 0;
% figure; plot(f0v', '+');
% TimeAxis = 1:size(f0v, 2);
% hold on; plot(TimeAxis, f0raw, 'go');
% 
% % If you are satisfied, press 'Esc' otherwise click on your mouse
% [xi,yi,but] = ginput(1);
% if but ~= 27
%     % Selects the upper and lower frequency limits to restrict the search
%     LowerB = SelectCurve(1, size(f0v, 2)); LowerB = interp1(LowerB(1, :), LowerB(2, :), TimeAxis); hold on; plot(TimeAxis, LowerB, '*');
%     UpperB = SelectCurve(1, size(f0v, 2)); UpperB = interp1(UpperB(1, :), UpperB(2, :), TimeAxis); hold on; plot(TimeAxis, UpperB, '*');
% 
%     for i = TimeAxis
%         f0raw(i) = 0;
%         for j = 1:size(f0v, 1)
%             vrvmin = Inf;
%             
%             % Selects the S/N optimum within the selected frequency search range
%             if f0v(j, i) > LowerB(i) && f0v(j, i) < UpperB(i)
%                 if f0raw(i) == 0 || vrv(j, i) < vrvmin
%                     f0raw(i) = f0v(j, i);
%                     vrvmin = vrv(j, i);
%                 end;
%             end;
...
%     end;
%     
%     % Reconstruct F0 (0: no candidates available)
%     nzf0 = f0raw(f0raw > 0); f0raw(1) = nzf0(1); f0raw(end) = nzf0(end);
%     nzf0 = f0raw(f0raw > 0); nzt = TimeAxis(f0raw > 0);
%     f0raw = interp1(nzt, nzf0, TimeAxis, 'spline');
%     
%     hold on; plot(TimeAxis, f0raw, 'ro');
% end;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
function xy = SelectCurve(xmin, xmax)
...
while but == 1
    [xi,yi,but] = ginput(1);
    plot(xi,yi,'r+'); plot(xi,yi,'ro');
    n = n+1;
    xy(:,n) = [xi;yi];
end;
[sxy, ixy] = sort(xy(1,2:end));
xy(:, 2:end) = xy(:, ixy + 1);
xy(1, 1) = xmin; xy(2, 1) = xy(2, 2);
xy(1, end + 1) = xmax; xy(2, end) = xy(2, end - 1);
